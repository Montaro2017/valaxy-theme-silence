...
const ccURL = computed(() => {
  const ccLang = siteConfig.value.license.language ?? 'zh-hans'
...
<template>
  <div class="silence-post-copyright">
    <p class="flex">
    ...
    <p class="flex">
      ...
      <span>本作品采用「<a :href="ccURL" target="_blank">CC {{ siteConfig.license.type.toUpperCase() }}</a>」许可协议进行许可。</span>
    </p>
  </div>
</template>

<style>
.silence-post-copyright {
...
.silence-post-copyright a {
  font-weight: 300;
  color: var(--text-color) !important;
}

.silence-post-copyright a:hover {
  opacity: 0.7;
}
</style>
